...
% and writes the feutures in to a file and save it
...
for i = 1:m
 ...
 for Label = 1:length(stats)
   ...
   % applied regionprops to the labeled objects
    ...
    %%
    % calculates the centroid ( x values and y values)
    % for all number of objects
  ...
  %%
    ...
    % checking the centroids of the same object in two differenc frames
    ...
    link =zeros([length(stats) 2]);
         ...
         % different colour for each object
         ...
         %% 
                ...
                % check minimum number of objects
                % bounding box for each object
                    ...
                    %%
                    % open new file
                    % writes the object feutures in to a file
                            ...
                            if (link(v,1)== v && link(v,2)== w)
                              ...
                              y(v)= (y_1(v)-y_2(w));
                               ...
                               if (y(v)> 0)
                                  ...
                                  fprintf(File,'displacement:%f\n', displacement(w));
                                  fprintf(File,'Angel:%0.00f\n', Angel(v));
